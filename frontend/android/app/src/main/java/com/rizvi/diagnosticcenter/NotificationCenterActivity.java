...
public class NotificationCenterActivity extends Activity {
    ...
    private void buildUi() {
        ...
        root.addView(header);
        ScrollView scroll = new ScrollView(this); list = new LinearLayout(this); list.setOrientation(LinearLayout.VERTICAL); list.setPadding(dp(16),dp(14),dp(16),dp(24)); scroll.addView(list); root.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));
        ...
        settings.addView(text("Notification settings",15,Color.rgb(15,23,42),true));
        ...
        root.addView(settings); setContentView(root); renderHistory();
    }
    private void addSwitch(LinearLayout parent,String label,boolean checked,final Toggle callback){ Switch sw=new Switch(this); sw.setText(label); sw.setTextSize(14); sw.setTextColor(Color.rgb(51,65,85)); sw.setChecked(checked); sw.setPadding(0,dp(4),0,dp(4)); sw.setOnCheckedChangeListener((button,isChecked)->callback.set(isChecked)); parent.addView(sw,new LinearLayout.LayoutParams(-1,-2)); }
    ...
    private void renderHistory() {
        ...
        for(int i=0;i<history.length();i++){JSONObject item=history.optJSONObject(i);if(item==null)continue;LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(14),dp(13),dp(14),dp(13));card.setBackgroundColor(item.optBoolean("read",false)?Color.WHITE:Color.rgb(239,246,255));card.addView(text(item.optString("title","Notification"),15,Color.rgb(15,23,42),true));TextView b=text(item.optString("message",""),13,Color.rgb(51,65,85),false);b.setPadding(0,dp(5),0,dp(3));card.addView(b);card.addView(text(df.format(new Date(item.optLong("timestamp",System.currentTimeMillis()))),11,Color.rgb(100,116,139),false));LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);cp.setMargins(0,0,0,dp(8));list.addView(card,cp);}
    }
}

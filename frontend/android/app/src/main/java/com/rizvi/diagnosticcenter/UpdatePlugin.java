...
public class UpdatePlugin extends Plugin {
    ...
    private String cleanReleaseNotes(String body){String s=String.valueOf(body==null?"":body).replace("\\r","").replace("\\n","\n").trim();if(s.isEmpty())return "Latest update from the current release.";String[] lines=s.split("\\n");boolean inNotes=false;StringBuilder out=new StringBuilder();for(String raw:lines){String line=raw.trim();if(line.isEmpty())continue;if(line.matches("(?i)^#{1,6}\\s*(build information|technical information|release information|verification|assets|installation|download|checksums?)\\s*$"))break;if(line.matches("(?i)^#{1,6}\\s*(what'?s new|latest update|changes?|release notes?)\\s*$")){inNotes=true;continue;}if(line.matches("(?i)^(version code|version name|apk sha-256|apk size|commit|package|workflow|artifact|release[- ]signed|automatically rebuilt)\\s*[:=].*$"))continue;if(line.matches("^#{1,6}\\s*.*$"))continue;if(!inNotes&&!line.matches("^[-*•]\\s+.*$"))continue;String clean=line.replaceFirst("^[-*•]\\s+","").replaceFirst("^\\d+[.)]\\s+","").trim();if(clean.isEmpty())continue;if(clean.matches("(?i)^(version code|version name|apk sha-256|apk size|commit|package|workflow|artifact|download|build information).*$"))continue;if(out.length()>0)out.append("\n");out.append("- ").append(clean);}return out.length()>0?out.toString():"Latest update from the current release.";}

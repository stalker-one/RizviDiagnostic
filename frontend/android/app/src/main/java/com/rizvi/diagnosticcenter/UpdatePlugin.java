...
public class UpdatePlugin extends Plugin {
    ...
    @PluginMethod public void checkForUpdate(PluginCall call) {
        executor.execute(() -> {
            try {
                ...
                c.setRequestProperty("Accept", "application/vnd.github+json"); c.setRequestProperty("User-Agent", "RizviDiagnosticCenter-Android-Updater/5"); c.connect();
                ...
                JSONObject release = new JSONObject(json.toString()); JSONArray assets = release.optJSONArray("assets");
                String expectedName = packageName.endsWith(".superadmin") ? "RizviDiagnosticSuperadmin-latest.apk" : "RizviDiagnosticCenter-latest.apk";
                ...
                result.put("available", true); result.put("versionCode", remoteCode); result.put("versionName", versionName.isEmpty() ? "1.0." + remoteCode : versionName); result.put("url", apk.optString("browser_download_url", ""));
                long size = apk.optLong("size", 0); result.put("sizeBytes", size); result.put("sizeMB", Math.round((size / 1024d / 1024d) * 10d) / 10d); result.put("commit", extractText(body, "commit\\s+([0-9a-f]{7,40})")); call.resolve(result);
            } catch (Exception e) { call.reject("Android update check failed: " + e.getMessage(), e); }
        });
    }

    private JSONObject findBestApk(JSONArray assets, String expectedName) { if (assets == null) return null; JSONObject fallback = null; long newest = Long.MIN_VALUE; for (int i=0;i<assets.length();i++) { JSONObject a=assets.optJSONObject(i); if(a==null)continue; String n=a.optString("name",""); if(expectedName.equals(n))return a; if(!n.toLowerCase().endsWith(".apk"))continue; long t=0; String u=a.optString("updated_at",""); if(!u.isEmpty()){try{t=Instant.parse(u).toEpochMilli();}catch(Exception ignored){}} if(fallback==null||t>newest){fallback=a;newest=t;}} return fallback; }
    ...
    @PluginMethod public void installApk(PluginCall call) {
        ...
        call.setKeepAlive(true); executor.execute(()->{File apk=new File(getContext().getCacheDir(),"rizvi-diagnostic-update.apk");try{if(apk.exists()&&!apk.delete())throw new IllegalStateException("Unable to clear the previous update file.");HttpURLConnection c=(HttpURLConnection)new URL(url).openConnection();c.setConnectTimeout(15000);c.setReadTimeout(120000);c.setInstanceFollowRedirects(true);c.setRequestProperty("User-Agent","RizviDiagnosticCenter-Android-Updater/5");c.connect();int response=c.getResponseCode();if(response<200||response>=300)throw new IllegalStateException("Update server returned HTTP "+response);long total=c.getContentLengthLong();try{InputStream input=c.getInputStream();FileOutputStream output=new FileOutputStream(apk);byte[] buffer=new byte[16384];long done=0;int n;int last=-1;while((n=input.read(buffer))!=-1){output.write(buffer,0,n);done+=n;int percent=total>0?(int)Math.min(99,(done*100L)/total):-1;if(percent!=last){last=percent;JSObject progress=new JSObject();progress.put("percent",percent);progress.put("downloadedBytes",done);progress.put("totalBytes",total);notifyListeners("updateProgress",progress);}}input.close();output.close();}finally{c.disconnect();}if(!apk.isFile()||apk.length()<10000)throw new IllegalStateException("Downloaded update APK is incomplete.");verifyDownloadedApk(apk);JSObject complete=new JSObject();complete.put("percent",100);complete.put("downloadedBytes",apk.length());complete.put("totalBytes",total);notifyListeners("updateProgress",complete);getActivity().runOnUiThread(()->{try{Uri uri=FileProvider.getUriForFile(getContext(),getContext().getPackageName()+".fileprovider",apk);Intent intent=new Intent(Intent.ACTION_VIEW);intent.setDataAndType(uri,"application/vnd.android.package-archive");intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_ACTIVITY_NEW_TASK);getActivity().startActivity(intent);call.resolve();}catch(Exception e){call.reject("Unable to start Android update installer: "+e.getMessage(),e);}finally{call.setKeepAlive(false);}});}catch(Exception e){getActivity().runOnUiThread(()->{call.reject("Update verification/download failed: "+e.getMessage(),e);call.setKeepAlive(false);});}});
    }

...
public class UpdatePlugin extends Plugin {
    ...
    @PluginMethod public void getVersion(PluginCall call) {
        try {
            ...
            JSObject result = new JSObject(); result.put("versionCode", versionCode); result.put("versionName", info.versionName == null ? "" : info.versionName); call.resolve(result);
    ...
    @PluginMethod public void checkForUpdate(PluginCall call) {
        executor.execute(() -> {
            try {
                ...
                URL apiUrl = new URL("https://api.github.com/repos/stalker-one/RizviDiagnostic/releases?per_page=20&_=" + System.currentTimeMillis());
                HttpURLConnection connection = (HttpURLConnection) apiUrl.openConnection();
                connection.setConnectTimeout(15000); connection.setReadTimeout(30000); connection.setUseCaches(false);
                connection.setRequestProperty("Accept", "application/vnd.github+json"); connection.setRequestProperty("User-Agent", "RizviDiagnosticCenter-Android-Updater"); connection.connect();
                if (connection.getResponseCode() < 200 || connection.getResponseCode() >= 300) throw new IllegalStateException("GitHub returned HTTP " + connection.getResponseCode());
                StringBuilder json = new StringBuilder();
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream(), StandardCharsets.UTF_8))) { String line; while ((line = reader.readLine()) != null) json.append(line); }
                finally { connection.disconnect(); }

                JSONArray releases = new JSONArray(json.toString()); JSONObject selected = null;
                for (int i = 0; i < releases.length(); i++) { JSONObject release = releases.optJSONObject(i); if (release == null || release.optBoolean("draft", false) || release.optBoolean("prerelease", false)) continue; if (tag.equals(release.optString("tag_name", ""))) { selected = release; break; } }
                JSObject result = new JSObject(); result.put("available", false); result.put("installedVersionCode", installedCode); result.put("installedVersionName", installed.versionName == null ? "" : installed.versionName); result.put("packageName", packageName); result.put("tag", tag);
                if (selected == null) { call.resolve(result); return; }
                JSONObject apk = null; JSONArray assets = selected.optJSONArray("assets");
                if (assets != null) for (int i = 0; i < assets.length(); i++) { JSONObject asset = assets.optJSONObject(i); if (asset != null && asset.optString("name", "").toLowerCase().endsWith(".apk")) { apk = asset; break; } }
                if (apk == null) { call.resolve(result); return; }
                String body = selected.optString("body", ""); long remoteCode = extractNumber(body, "Version code\\s*:\\s*(\\d+)");
                ...
                result.put("available", true); result.put("versionCode", remoteCode); result.put("versionName", versionName.isEmpty() ? "1.0." + Math.max(0, remoteCode - 1) : versionName); result.put("url", apk.optString("browser_download_url", "")); result.put("releaseName", selected.optString("name", tag)); call.resolve(result);
    ...
    @PluginMethod public void installApk(PluginCall call) {
        String url = call.getString("url", ""); if (url == null || url.trim().isEmpty()) { call.reject("Update download URL is missing."); return; } if (!url.startsWith("https://")) { call.reject("Only secure HTTPS update URLs are allowed."); return; }
        ...
        call.setKeepAlive(true); executor.execute(() -> { File apk = new File(getContext().getCacheDir(), "rizvi-diagnostic-update.apk"); try {
            if (apk.exists() && !apk.delete()) throw new IllegalStateException("Unable to clear the previous update file.");
            HttpURLConnection connection = (HttpURLConnection) new URL(url).openConnection(); connection.setConnectTimeout(15000); connection.setReadTimeout(120000); connection.setInstanceFollowRedirects(true); connection.setRequestProperty("User-Agent", "RizviDiagnosticCenter-Android-Updater"); connection.connect();
            ...
            try (InputStream input = connection.getInputStream(); FileOutputStream output = new FileOutputStream(apk)) { byte[] buffer = new byte[8192]; int count; while ((count = input.read(buffer)) != -1) output.write(buffer, 0, count); } finally { connection.disconnect(); }
            if (!apk.isFile() || apk.length() < 10000) throw new IllegalStateException("Downloaded update APK is incomplete."); verifyDownloadedApk(apk);
            getActivity().runOnUiThread(() -> { try { Uri apkUri = FileProvider.getUriForFile(getContext(), getContext().getPackageName() + ".fileprovider", apk); Intent installIntent = new Intent(Intent.ACTION_VIEW); installIntent.setDataAndType(apkUri, "application/vnd.android.package-archive"); installIntent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_ACTIVITY_NEW_TASK); getActivity().startActivity(installIntent); call.resolve(); } catch (Exception e) { call.reject("Unable to start Android update installer: " + e.getMessage(), e); } finally { call.setKeepAlive(false); } });

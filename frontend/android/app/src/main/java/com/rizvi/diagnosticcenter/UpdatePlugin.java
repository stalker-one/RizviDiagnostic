...
public class UpdatePlugin extends Plugin {
    ...
    @PluginMethod public void checkForUpdate(PluginCall call) {
        executor.execute(() -> {
            try {
                ...
                // Query the exact moving release tag instead of paginating through releases.
                ...
                c.setRequestProperty("User-Agent", "RizviDiagnosticCenter-Android-Updater/2");
                c.connect();
                ...
                if (assets != null) {
                    for (int i = 0; i < assets.length(); i++) {
                        ...
                        if (a != null && a.optString("name", "").toLowerCase().endsWith(".apk")) { apk = a; break; }
                    }
                }
                ...
                // Never hide an update merely because version metadata is malformed.
                // If the release contains an APK but no numeric metadata, report the issue explicitly.
    ...
    @PluginMethod public void installApk(PluginCall call) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && !getContext().getPackageManager().canRequestPackageInstalls()) {
            try { getActivity().startActivity(new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES, Uri.parse("package:" + getContext().getPackageName()))); call.reject("Please allow this app to install updates, then tap Update Now again."); }
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                c.setConnectTimeout(15000); c.setReadTimeout(120000); c.setInstanceFollowRedirects(true); c.setRequestProperty("User-Agent", "RizviDiagnosticCenter-Android-Updater/2"); c.connect();
                if (c.getResponseCode() < 200 || c.getResponseCode() >= 300) throw new IllegalStateException("Update server returned HTTP " + c.getResponseCode());
                try (InputStream input = c.getInputStream(); FileOutputStream output = new FileOutputStream(apk)) { byte[] buffer = new byte[8192]; int n; while ((n = input.read(buffer)) != -1) output.write(buffer, 0, n); }
                finally { c.disconnect(); }
                ...
                getActivity().runOnUiThread(() -> {
                    try {
                        ...
                        Intent intent = new Intent(Intent.ACTION_VIEW); intent.setDataAndType(uri, "application/vnd.android.package-archive"); intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_ACTIVITY_NEW_TASK); getActivity().startActivity(intent); call.resolve();
    ...
    private void verifyDownloadedApk(File apk) throws Exception {
        ...
        PackageInfo info = Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU ? pm.getPackageArchiveInfo(apk.getAbsolutePath(), PackageManager.PackageInfoFlags.of(PackageManager.GET_SIGNING_CERTIFICATES)) : pm.getPackageArchiveInfo(apk.getAbsolutePath(), PackageManager.GET_SIGNATURES);

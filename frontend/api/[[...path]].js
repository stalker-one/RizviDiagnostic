...
// Keeping the original request path is important because Express mounts
// routes under /api/* (for example /api/auth/login).
...
function ensureDbReady() {
  if (!dbReady) {
    dbReady = Promise.resolve()
      ...
      .catch((err) => {
        console.warn('[vercel] MongoDB init failed:', err.message);
      });
  }
  return dbReady;
}

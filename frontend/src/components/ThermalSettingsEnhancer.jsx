...
function unlockThermalFontSize(){
 const labels=Array.from(document.querySelectorAll('label'));
 labels.forEach(label=>{
  if(!/font size\s*\(px\)/i.test(label.textContent||''))return;
  const input=label.querySelector('input')||label.parentElement?.querySelector('input');
  if(input&&input.type==='number'){input.removeAttribute('max');input.setAttribute('step','1');}
 });
}
...
export default function ThermalSettingsEnhancer(){
 ...
 return createPortal(<div className="border-t border-slate-100 pt-4 mt-4 space-y-5">
  ...
  <div className="border-t border-slate-100 pt-4"><h4 className="font-semibold text-slate-700">Paid Stamp — Thermal Receipt</h4><p className="text-xs text-slate-500 mt-1">Choose the stamp position, design, rotation and CSS-style offsets.</p></div>

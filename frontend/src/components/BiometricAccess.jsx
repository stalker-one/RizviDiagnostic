...
export default function BiometricAccess() {
  ...
  useEffect(() => {
    setMessage(''); setSuccess(false); setVisible(false); autoLoginStarted.current = false;
    refresh();
  }, [location.pathname]);
  ...
  const biometricLogin = async () => {
    ...
    try {
      ...
      if (err?.response?.status === 401) {
        ...
        setMessage('Fingerprint session expired. Please sign in with your email and password.');
      } else {
        const text = err?.message || 'Fingerprint was not verified. Please use your email and password to sign in.';
        setMessage(/cancel|negative|use password/i.test(text) ? 'Fingerprint was not verified. Please use your email and password to sign in.' : text);
      }
  ...
  const enable = async () => {
    ...
    if (busy) return;
    setBusy(true); setSuccess(false); setMessage('Confirm your enrolled fingerprint in the Android system prompt.');
    try {

...
export default function PrintThermalInvoice({invoice,settings}){
 ...
 const marginTop=numberSetting(settings?.thermalMarginTop,0,0,20),marginBottom=numberSetting(settings?.thermalMarginBottom,0,0,30);
 ...
 const stampAfterBookedBy=settings?.stampPosition==='after-booked-by';
 ...
 return <div className="thermal-invoice relative" style={contentStyle}>
   {showStamp&&!stampAfterBookedBy&&<PaidStamp settings={settings} compact/>}
   ...
   {bookedBy}
   {showStamp&&stampAfterBookedBy&&<div style={{position:'relative',minHeight:'55px',marginTop:`${sectionSpacing}px`,display:'flex',justifyContent:'center',alignItems:'center'}}><PaidStamp settings={{...settings,stampPosition:'center'}} compact/></div>}
 </div>;
}

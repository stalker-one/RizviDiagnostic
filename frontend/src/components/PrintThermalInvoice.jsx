...
export default function PrintThermalInvoice({invoice,settings}){
 ...
 const bookedBy=<div style={{textAlign:'center',fontSize:`${Math.max(8,fontSize-1)}px`,color:'#888',marginTop:`${sectionSpacing}px`,width:'100%',overflowWrap:'anywhere'}}>Booked by: {safeText(invoice.createdByName)}</div>;
 ...
 return <><style>{pageStyle}</style><div className="thermal-invoice relative" style={contentStyle}>
   {showStamp&&!stampAfterBookedBy&&<PaidStamp settings={settings} compact variant="thermal"/>}
   ...
   <div style={{marginTop:`${tableSpacing}px`,borderTop:'1px solid black',paddingTop:`${sectionSpacing}px`,width:'100%',maxWidth:'100%'}}>
     {Number(invoice.discountAmount)>0 && (
       <div className="thermal-total" style={{display:'flex',justifyContent:'space-between',width:'100%',gap:'12px'}}>
         <span className="thermal-total-label" style={{flex:'0 0 auto'}}>Discount:</span>
         <span className="thermal-total-value" style={{flex:'0 0 auto',textAlign:'right'}}>{amount(invoice.discountAmount)}</span>
       </div>
     )}
     <div className="thermal-total" style={{display:'flex',justifyContent:'space-between',width:'100%',gap:'12px',fontWeight:'bold'}}>
       <span className="thermal-total-label" style={{flex:'0 0 auto'}}>Total:</span>
       <span className="thermal-total-value" style={{flex:'0 0 auto',textAlign:'right'}}>{amount(invoice.total)}</span>
     </div>
     <div className="thermal-total" style={{display:'flex',justifyContent:'space-between',width:'100%',gap:'12px'}}>
       <span className="thermal-total-label" style={{flex:'0 0 auto'}}>Payment Mode:</span>
       <span className="thermal-total-value" style={{flex:'0 0 auto',textAlign:'right'}}>{safeText(invoice.paymentMode)}</span>
     </div>
     <div className="thermal-total" style={{display:'flex',justifyContent:'space-between',width:'100%',gap:'12px'}}>
       <span className="thermal-total-label" style={{flex:'0 0 auto'}}>Paid Amount:</span>
       <span className="thermal-total-value" style={{flex:'0 0 auto',textAlign:'right'}}>{amount(invoice.amountPaid)}</span>
     </div>
     <div className="thermal-total" style={{display:'flex',justifyContent:'space-between',width:'100%',gap:'12px',fontWeight:'bold'}}>
       <span className="thermal-total-label" style={{flex:'0 0 auto'}}>Due Amount:</span>
       <span className="thermal-total-value" style={{flex:'0 0 auto',textAlign:'right'}}>Rs. {amount(invoice.dueAmount)}</span>
     </div>
   </div>
   ...
   {bookedBy}
   {showStamp&&stampAfterBookedBy&&<div style={{position:'relative',minHeight:'55px',marginTop:`${sectionSpacing}px`,display:'flex',justifyContent:'center',alignItems:'center'}}><PaidStamp settings={{...settings,thermalStampPosition:'center'}} compact variant="thermal"/></div>}
 </div></>;
}

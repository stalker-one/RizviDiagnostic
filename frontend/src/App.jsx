...
const REALTIME_RECONNECT_INITIAL_MS = 1000;
...
export default function App(){
 const{user}=useAuth();
 const[androidUpdate,setAndroidUpdate]=useState(null);const[androidUpdateBusy,setAndroidUpdateBusy]=useState(false);const[androidUpdateError,setAndroidUpdateError]=useState('');const[androidUpdateChecking,setAndroidUpdateChecking]=useState(false);const[androidUpdateProgress,setAndroidUpdateProgress]=useState({percent:0,downloadedBytes:0,totalBytes:0});
 const checkingAndroidUpdate=useRef(false);const retryCheckRef=useRef(null);const dismissedUpdateCodeRef=useRef(null);
 ...
 useEffect(()=>{let stopped=false;let reconnectTimer=null;let reconnectDelay=REALTIME_RECONNECT_INITIAL_MS;let controller=null;const apiBase=(import.meta.env.VITE_API_BASE_URL||'/api').replace(/\/$/,'');const parseSseBlock=(block)=>{let eventName='message';const data=[];for(const rawLine of block.split(/\r?\n/)){const line=rawLine.trimEnd();if(line.startsWith('event:'))eventName=line.slice(6).trim();else if(line.startsWith('data:'))data.push(line.slice(5).trimStart());}if(!data.length)return null;try{return{eventName,payload:JSON.parse(data.join('\\n'))};}catch{return null;}};const connect=async()=>{if(stopped||!localStorage.getItem('rdc_token'))return;controller=new AbortController();try{const response=await fetch(`${apiBase}/sync/events`,{headers:{Authorization:`Bearer ${localStorage.getItem('rdc_token')}`,'Cache-Control':'no-cache'},cache:'no-store',signal:controller.signal});if(!response.ok)throw new Error(`Realtime connection failed (${response.status})`);if(!response.body)throw new Error('Realtime streaming is not supported by this browser.');reconnectDelay=REALTIME_RECONNECT_INITIAL_MS;const reader=response.body.getReader();const decoder=new TextDecoder();let buffer='';while(!stopped){const{value,done}=await reader.read();if(done)break;buffer+=decoder.decode(value,{stream:true});const blocks=buffer.split(/\r?\n\r?\n/);buffer=blocks.pop()||'';for(const block of blocks){const parsed=parseSseBlock(block);if(parsed?.eventName==='data.changed'&&parsed.payload?.type==='data.changed')window.dispatchEvent(new CustomEvent('rdc:data-changed',{detail:parsed.payload}));else if(parsed?.eventName==='ready')window.dispatchEvent(new CustomEvent('rdc:data-changed',{detail:{type:'data.changed',tables:['users','patients','procedures','referrals','doctors','invoices','settings','counters','biometricSessions','pushTokens'],ready:true}}));}}if(!stopped&&!controller.signal.aborted){reconnectTimer=window.setTimeout(connect,reconnectDelay);reconnectDelay=Math.min(reconnectDelay*2,REALTIME_RECONNECT_MAX_MS);}}catch(error){if(!stopped&&!controller.signal.aborted){console.warn('[realtime] stream disconnected:',error.message);reconnectTimer=window.setTimeout(connect,reconnectDelay);reconnectDelay=Math.min(reconnectDelay*2,REALTIME_RECONNECT_MAX_MS);}}finally{controller=null;}};connect();return()=>{stopped=true;if(reconnectTimer)window.clearTimeout(reconnectTimer);controller?.abort();};},[user?.id]);
 ...
 useEffect(()=>{if(!user||!Capacitor.isNativePlatform()||Capacitor.getPlatform()!=='android')return undefined;let cancelled=false;(async()=>{try{if(typeof AndroidUpdate.ensureNotificationPermission==='function')await AndroidUpdate.ensureNotificationPermission().catch(()=>{});if(typeof AndroidUpdate.getFcmToken!=='function')return;const{token}=await AndroidUpdate.getFcmToken();if(cancelled||!token)return;await api.post('/push/register-token',{token,appVariant:IS_SUPERADMIN_APP?'superadmin':'staff'});}catch{}})();return()=>{cancelled=true;};},[user]);

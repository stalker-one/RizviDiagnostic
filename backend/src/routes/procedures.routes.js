...
router.post('/', requireRole('admin', 'superadmin'), (req, res) => {
  ...
  if (!name || price === undefined) {
    return res.status(400).json({ message: 'Procedure name and price are required.' });
  }
  const procedures = readTable('procedures');
  const newProc = {
    id: generateId('proc'),
    ...
    doctorsSharePercent: Number(doctorsSharePercent) || 0,
    active: true,
...
// Bulk import for the Radiology Procedure List — accepts an array of rows
// (as parsed from an imported CSV/Excel export) and creates one procedure per
// valid row. Existing procedures with the same name (case-insensitive) are
// updated in place instead of duplicated, so re-importing an updated price
// list is safe to repeat.
router.post('/import', requireRole('admin', 'superadmin'), (req, res) => {
  ...
  rows.forEach((row) => {
    ...
    if (existing) {
      ...
      existing.doctorsSharePercent = doctorsSharePercent;

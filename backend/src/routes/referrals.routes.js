...
router.post('/', requireRole('admin', 'superadmin'), (req, res) => {
  ...
  const newRef = { id: generateId('ref'), name, department: department || '', phone: phone || '', address: address || '', sharePercent: Number(sharePercent) || 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
...
router.delete('/:id', requireRole('admin', 'superadmin'), async (req, res) => {
  const referrals = await getFreshTable('referrals', readTable('referrals'));

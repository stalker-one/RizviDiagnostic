...
const BIOMETRIC_TOKEN_FILE = 'biometric-login.dat';
...
if (!gotLock) app.quit(); else {
  ...
  async function startBackend() { const envPath = path.join(BACKEND_ROOT, '.env'); if (fs.existsSync(envPath)) require('dotenv').config({ path: envPath, override: false }); const dataDir = path.join(app.getPath('userData'), 'data'); fs.mkdirSync(dataDir, { recursive: true }); process.env.RIZVI_DATA_DIR = dataDir; logToFile(`Backend root: ${BACKEND_ROOT}`); logToFile(`Backend data directory: ${dataDir}`); logToFile(`MongoDB configured: ${Boolean(process.env.MONGODB_URI || process.env.MONGODB_URI_2 || process.env.MONGODB_URI_3)}`); const { start } = require(path.join(BACKEND_ROOT, 'src', 'server.js')); await start(); }
  function createWindow() { const port = process.env.PORT || 5000; mainWindow = new BrowserWindow({ width: 1360, height: 860, minWidth: 1024, minHeight: 640, title: process.env.CLINIC_NAME || 'Rizvi Diagnostic Center', icon: path.join(__dirname, 'build', 'Rizvi-Logo-favicon.png'), webPreferences: { contextIsolation: true, nodeIntegration: false, preload: path.join(__dirname, 'preload.js') }, show: false }); Menu.setApplicationMenu(null); mainWindow.once('ready-to-show', () => mainWindow.show()); mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => { logToFile(`Page failed to load: ${errorCode} ${errorDescription}`); showFatalError('Rizvi Diagnostic Center failed to load', new Error(`${errorDescription} (${errorCode}) while loading http://localhost:${port}`)); }); mainWindow.loadURL(`http://localhost:${port}`); mainWindow.on('closed', () => { mainWindow = null; }); }

...
function requestJson(url, redirectCount = 0) { return new Promise((resolve, reject) => {
  ...
  const request = https.get(requestUrl, { headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'Rizvi-Diagnostic-Center-Desktop', 'Cache-Control': 'no-cache', Pragma: 'no-cache' } }, (response) => {
    if ([301, 302, 303, 307, 308].includes(response.statusCode) && response.headers.location) {
      response.resume();
      requestJson(response.headers.location, redirectCount + 1).then(resolve).catch(reject);
      return;
    }
    let body = ''; response.setEncoding('utf8'); response.on('data', (chunk) => { body += chunk; }); response.on('end', () => {
      if (response.statusCode < 200 || response.statusCode >= 300) { reject(new Error(`GitHub Windows release check returned HTTP ${response.statusCode}`)); return; }
...
function getWindowsInstallerAsset(release, expectedVersion) {
  ...
  if (expectedVersion) { const exactName = `Rizvi-Diagnostic-Center-Setup-${String(expectedVersion).replace(/^v/i, '')}.exe`.toLowerCase(); const exact = assets.find((asset) => String(asset.name || '').toLowerCase() === exactName); if (exact) return exact; }
  const versioned = assets.map((asset) => { const match = /^Rizvi-Diagnostic-Center-Setup-(\d+\.\d+\.\d+)\.exe$/i.exec(String(asset.name || '')); return match ? { asset, version: match[1] } : null; }).filter(Boolean).sort((a, b) => compareVersions(b.version, a.version));
...
function getHighestInstallerVersion(release) {
  const assets = Array.isArray(release.assets) ? release.assets : [];
  return assets.map((asset) => { const match = /^Rizvi-Diagnostic-Center-Setup-(\d+\.\d+\.\d+)\.exe$/i.exec(String(asset.name || '')); return match ? match[1] : null; }).filter(Boolean).sort(compareVersions).pop() || '';
}
function getBuildVersionFromRelease(release) {
  ...
  return requestJson(manifest.browser_download_url).catch((error) => { logToFile(`[update] windows-version.json could not be read: ${error.message}; falling back to installer asset version.`); return null; });
}
...
async function checkForLatestWindowsUpdate(showNoUpdate = false) {
  ...
  try {
    ...
    logToFile(`[update] Checked Windows release ${release.tag_name}: installed=${currentVersion}, manifest=${manifest?.version || 'none'}, installer=${installerVersion || 'none'}, latest=${latestVersion}, asset=${asset.name}`);
...
function setupAutoUpdate() {
  ...
  if (mainWindow.isVisible()) runInitialCheck(); else mainWindow.once('ready-to-show', runInitialCheck);
